...
function inserttable(funcname, usage, ret, memo, arrPara, example, illu, divFuncTable) {
    ...
    str += '<td style="border-left:0px;">' + illu + '<a href="#func_' + divFuncTable + funcname + '"><img border=0 src="arrowup.gif" title="�ص�Ŀ¼"/> </a> </td></tr>';

    var paras = arrPara.length;
    if (paras == 0)
        str += '<tr><td align=right>����</td><td colspan=2>(��)</td></tr>';
    else if (paras == 1)
        str += '<tr><td align=right>����</td><td colspan=2>' + arrPara[0] + '</td></tr>';
    else {
        for (var i = 1; i <= paras; i++) {
            str += '<tr><td align=right>����' + i + '</td><td colspan=2>' + arrPara[i - 1] + '</td></tr>';
        }
    }

    if (ret == "") ret = "(��)";
    str += '<tr><td align=right>����ֵ</td><td colspan=2>' + ret + '</td></tr>';
    if (memo != "") str += '<tr><td align=right>��ע</td><td colspan=2>' + memo + '</td></tr>';
    if (example != "") str += '<tr bgColor=yellow><td align=right>ʾ��</td><td colspan=2>' + example + '</td></tr>';
    str += '</table><br>';
    return str;
}
...
function genfunctable(divFuncTable, divFunc, xmldoc) {
    ...
    str += '<tr bgcolor=#eaeaea><th>����</th><th>������</th><th>��;</th></tr>';
    for (i = 0; i < cats; i++) {
        var cat = x[i];
...
function geneventtable(divTag, xmldoc, nodename) {
    ...
    str += '<tr bgcolor=#eaeaea><td>Event</td><td>����</td><td>p1</td><td>p2</td><td>p3</td><td>p4</td></tr>';
