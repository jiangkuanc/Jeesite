...
function inserttable(funcname, usage, ret, memo, arrPara, example, illu, divFuncTable) {
    ...
    str += '<td style="border-left:0px;">' + illu + '<a href="#func_' + divFuncTable + funcname + '"><img border=0 src="arrowup.gif" title="�^��ؿ�"/> </a> </td></tr>';

    var paras = arrPara.length;
    if (paras == 0)
        str += '<tr><td align=right>�Ѽ�</td><td colspan=2>(�L)</td></tr>';
    else if (paras == 1)
        str += '<tr><td align=right>�Ѽ�</td><td colspan=2>' + arrPara[0] + '</td></tr>';
    else {
        for (var i = 1; i <= paras; i++) {
            str += '<tr><td align=right>�Ѽ�' + i + '</td><td colspan=2>' + arrPara[i - 1] + '</td></tr>';
        }
    }

    if (ret == "") ret = "(�L)";
    str += '<tr><td align=right>��^��</td><td colspan=2>' + ret + '</td></tr>';
    if (memo != "") str += '<tr><td align=right>�Ƶ�</td><td colspan=2>' + memo + '</td></tr>';
    if (example != "") str += '<tr bgColor=yellow><td align=right>�ܨ�</td><td colspan=2>' + example + '</td></tr>';
    str += '</table><br>';
    return str;
}
...
function genfunctable(divFuncTable, divFunc, xmldoc) {
    ...
    str += '<tr bgcolor=#eaeaea><th>����</th><th>��ƦW</th><th>�γ~</th></tr>';
    for (i = 0; i < cats; i++) {
        var cat = x[i];
