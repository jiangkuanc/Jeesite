...
function gentr(hasexample, name, meaning, val, defaultval, sample, inc, exc, memo, lastGroup, groups, groupcurrent, groupcolor) {
    ...
    if (sample == '')
        ...
        sample = '<img src="document.bmp" title="��μ���ʾҳ: ' + sample + '">';
    if (inc == "Edit*") inc = "Edit��EditWithButton�ĵ��������Ķ���";
    if (exc == "Edit*") exc = "Edit��EditWithButton�ĵ��������Ķ���";

    var arrMemo = new Array(0);
    if (memo != "") arrMemo = memo.split('<br>');
    if (inc != "") arrMemo.push('������' + inc);
    if (exc != "") arrMemo.push('��' + exc + '��Ч;');
    memo = "";
    if (arrMemo.length == 1)
        memo = 'ע: ' + arrMemo[0];
    else {
...
function genproptable(divTag, xmldoc, zebracolor) {
    ...
    var x = xmldoc.getElementsByTagName('elements');
    if (x == null) return;
    var content = "", sample = "";
    ...
    if (sample != "") str += '����<img src="document.bmp" title="��μ���ʾҳ' + sample + '">';
    str += '<br>';
    ...
    str += '<tr bgcolor=#e4e4e4><td colspan=' + cols + '>����: ' + meaning + '������: ' + number;
    if (content != "") str += ', ����: ' + content;
    str += '</td></tr>';
    str += '<tr bgcolor=#f1f1f1><td colspan=2>��������</td><td>����</td><td>����ֵ</td><td>Ĭ��ֵ</td>';
    if (hasexample) str += '<td>ʾ��</td>';
    str += '</tr>';
