...
<table id="contentTable" class="table table-bordered table-condensed">
    ...
    <tr>
        ...
        <th>评论IP</th>
        <th>评论时间</th>
        <c:if test="${comment.delFlag ne '1'}">
    ...
    <c:forEach items="${page.list}" var="comment">
        <tr>
            <td><p onclick="$('#c_${comment.id}').toggle()">${fns:abbr(fns:replaceHtml(comment.content),40)}</p></td>
            <!--onclick="return view(this.href);"  -->
            <td>
                <p <%-- href="${pageContext.request.contextPath}${fns:getFrontPath()}/view-${comment.category.id}-${comment.contentId}${fns:getUrlSuffix()}" --%>
                        title="${comment.title}">${fns:abbr(comment.title,40)}</p></td>
            <td>${comment.name}</td>
            <td>${comment.ip}</td>
            <td><fmt:formatDate value="${comment.createDate}" type="both"/></td>
            <c:if test="${comment.delFlag ne '1'}">
                <td><shiro:hasPermission name="cms:comment:edit">
                    <c:if test="${comment.delFlag eq '0'}"><a href="${ctx}/cms/comment/delete?id=${comment.id}"
                                                              onclick="return confirmx('确认要删除该评论吗？', this.href)">删除</a></c:if>
