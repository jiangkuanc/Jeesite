...
<ul class="nav nav-tabs">
    <li><a href="${ctx}/cms/article/?category.id=${article.category.id}">新闻列表</a></li>
    <li class="active"><a
            href="<c:url value='${fns:getAdminPath()}/cms/article/form?id=${article.id}&category.id=${article.category.id}'>
						<c:param name='category.name' value='${article.category.name}'/>
					</c:url>">新闻<shiro:hasPermission
            name="cms:article:edit">${not empty article.id?'修改':'添加'}</shiro:hasPermission>
        <shiro:lacksPermission name="cms:article:edit">查看</shiro:lacksPermission>
    </a>
    </li>
</ul>
<br/>
<form:form id="inputForm" enctype="multipart/form-data" modelAttribute="article" action="${ctx}/cms/article/save"
    ...
    <div class="control-group">
        <label class="control-label">新闻关键字:</label>
        <div class="controls">
