...
<div class="xyjyjt-content-mbx">
    <ol class="breadcrumb">
        ...
        <li class="active"><a href="#">一带一路项目招标</a></li>
    </ol>
</div>
<div class="contbox">
    ...
    <div class="xmzb_box mgtop_30 bgColor">
        ...
        <div class="xyjy-content-top-bot" style="margin-left:60px">
            <div id="itemContainer1">
                <c:forEach items="${fnc:getArticleList(site.id, 60, 1000, '')}" var="article">
                    <c:if test="${article.statusCode eq '5'}">
                        <div>
                            <div class="xyjy-content-top-bot-content pull-left" style="width:25%;">
                                <div class="xyjyjt-content-top-bot-content-left pull-left"
                                    ...
                                    <ul>
                                        <c:forEach var="img" items="${fn:split(article.image,'|') }" varStatus="status">
                                            <li class="pull-left">
                                                <a href="${ctx}/cms/cmsProjectDetils/selectCurrentDetail?pid=${article.id}">
                                                    <img alt="" style="width:180px;" src="${img}">
                                                </a>
                                            </li>
                                        </c:forEach>
                                        <div class="clearfix"></div>
                                    </ul>
                                        <%--  <img alt="" src="http://localhost:8080/jeesite/${fns:abbr(article.image,28)}">  --%>
                                    <p align="center"><a
                                            href="${ctx}/cms/cmsProjectDetils/selectCurrentDetail?pid=${article.id}"
                                            style="color:${article.color}"
                                            title="${article.title }">${fns:abbr(article.title,25)}</a></p>

                                </div>
                            </div>

                            <div class="xyjyjt-content-top-bot-content-right pull-left"
                                 style="margin-top:30px; margin-left:0px;line-height:30px;">
                                <p>项目名称：<a href="${ctx}/cms/cmsProjectDetils/selectCurrentDetail?pid=${article.id}"
                                           style="color:${article.color}">${fns:abbr(article.title,60)}</a></p>
                                <p>项目时间：<fmt:formatDate value="${article.createDate}" pattern="yyyy-MM-dd"/></span></p>
                                <p>项目规模：<a href="${article.url}"
                                           style="color:${article.color}">${fns:abbr(article.projectScale,60)}</a></p>
                                <p>相关企业：<a href="${article.url}"
                                           style="color:${article.color}">${fns:abbr(article.relatedEnterprises,60)}</a>
                                </p>
