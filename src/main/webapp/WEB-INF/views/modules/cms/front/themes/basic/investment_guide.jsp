...
<div class="xyjyjt-content-mbx">
    <ol class="breadcrumb">
        ...
        <li class="active"><a href="#">投资指南</a></li>
    </ol>
</div>
<!--信息产品-->
<div class="index-xinyong bgColor">
    ...
    <div class="table-bot">
        <table class="table table_xxcp table-hover">
            <thead>
            <tr>
                <th class="product">投资指南类型</th>
                <th>国别</th>
            </tr>
            </thead>
            <tbody id="itemContainer">
            <tr class="kheight"></tr>
            <c:forEach items="${fnc:getArticleList(site.id, 64, 1000, '')}" var="article">
                <c:if test="${article.statusCode eq '5'}">
                    <tr>
                        <td class="product_text"><a
                                ...
                                style="color:${article.color}">${fns:abbr(article.title,64)}</a></td>
                        <td><span><a style="color:${article.color}">${fns:abbr(article.country,64)}</a></span></td>
                    </tr>
                </c:if>
            </c:forEach>
            </tbody>
