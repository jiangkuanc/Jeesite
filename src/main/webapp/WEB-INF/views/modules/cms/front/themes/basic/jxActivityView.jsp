...
<div class="row">
    <div class="xyjyjt-content">
        <c:forEach items="${jxActivityDetails}" var="jxActivityDetails" varStatus="status">
            <ul class="jyjtList clearfix">
                <li class="pull-left" style="font-size:25px" id="title">${fns:abbr(jxActivityDetails.title,30)}</li>
                <li class="pull-left" style="margin-left:40px">活动讲师：${jxActivityDetails.speaker}</li>
                <li class="pull-left" style="margin-left:50px">活动时间：<fmt:formatDate
                        value="${jxActivityDetails.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/>&nbsp;至
                    <fmt:formatDate value="${jxActivityDetails.weightDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
                </li>
            </ul>
            <div class="prediv" style="width:100%;word-wrap: break-word;"><span>关键字：</span>${jxActivityDetails.keywords}
            </div>
            <div class="prediv" style="width:100%;word-wrap: break-word;">
                <span>活动摘要：</span>${jxActivityDetails.description}</div>
            <div class="prediv"
                 style="width:100%;word-wrap: break-word;padding:0 50px">${jxActivityDetails.articleData.content}</div>
        </c:forEach>
