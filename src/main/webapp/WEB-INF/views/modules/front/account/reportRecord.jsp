...
<div class="zh-right pull-left">
    <div class="tabs " id="tabs" style="width:100%">
        <ul>
            ...
            <li id="changeTwo" class="two border current currents" style="width:30%"><span
                    class="border-top">我的举报记录</span></li>
            <div style="clear:both;"></div>
        </ul>

        <table class="table zh-right-bot-table">
            <thead>

            <tr class="active list-th">
                <td style="width:8%;">序号</td>
                <td name="reportInfoType" style="word-wrap:break-word;word-break:break-all">信用记录类型</td>
                <td name="attachment" style="word-wrap:break-word;word-break:break-all">附件</td>
                <td name="reportTime">举报时间</td>
                <td name="auditStatus" style="width:10%;">举报状态</td>
                <td>操作</td>
            </tr>
            </thead>
            <tbody id="itemContainer">
            <c:forEach items="${creditReportInfoList}" var="creditReportInfo" varStatus="status">
                <tr>
                    <td>${status.count}</td>
                    <td>${fns:getDictLabel(creditReportInfo.reportInfoType,'info_type','')}</td>
                    <td>

                        <c:if test="${creditReportInfo.attachment!=null}">
                            <a style="color:blue"
                               href="${ctx}/creditmanage/creditReportInfo/attachmentDownload?attachmentName=${creditReportInfo.attachmentName}&attachment=${creditReportInfo.attachment}">附件下载</a>
                        </c:if>
                    </td>
                    <td><fmt:formatDate value="${creditReportInfo.reportTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                        <%-- <td>${p2pReportInformationAudit.reportContent}</td> --%>
                ...
                <table class="table table-striped table-bordered table-condensed mytable" style="text-align:center">
                    ...
                    <tr>
                        ...
                        <td id="attachmentName">
                            ...
                            <c:if test="${creditReportInfo.attachment!=null}">
                                <a href="${ctx}/creditmanage/creditReportInfo/attachmentDownload?attachmentName=${creditReportInfo.attachmentName}&attachment=${creditReportInfo.attachment}">附件下载</a>
                            </c:if>
                        </td>
                    </tr>
                    </tbody>
...
<script type="text/javascript">
    ...
    function view(index) {
        ...
        {
            $('#reportInfoType').text('${fns:getDictLabel(creditReportInfo.reportInfoType,'info_type','')}');
            $('#reportReason').text('${fns:getDictLabel(creditReportInfo.reportReason,'report_category','')}');
            $('#reportExplain').text('${creditReportInfo.reportExplain}');
            $('#attachmentName').html('${creditReportInfo.attachmentName}&nbsp;&nbsp;&nbsp;' +
                '<c:if test="${creditReportInfo.attachment!=null}">' +
                '<a href="${ctx}/creditmanage/creditReportInfo/attachmentDownload?attachmentName=${creditReportInfo.attachmentName}&attachment=${creditReportInfo.attachment}">附件下载</a>' +
                '</c:if>');
            $('#reportTime').html('<fmt:formatDate value="${creditReportInfo.reportTime}" pattern="yyyy-MM-dd HH:mm:ss"/>');
            $('#auditStatus').text('${fns:getDictLabel(creditReportInfo.auditStatus,'report_audit_state','')}');
        }
        </c:forEach>
        $('#exampleModal').modal();
