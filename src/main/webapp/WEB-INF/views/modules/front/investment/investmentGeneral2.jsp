    ...
    <div class="zh-right-bottom">
        ...
        <table class="table table-hover tzzl-table table-striped table-condensed"
            ...
            <tr class="active list-th">
                ...
                <th width="9.9%">信用级别</th>
                <th width="13.8%">募集截止日期</th>
                <th width="10.80%">收益率(年化)</th>
                <th width="15.3%">投资额度(元)</th>
                <th width="12.4%">投资时间</th>
                <th width="10.5%">债项状态</th>
                <th width="15.63%">划款时间(计息时间)</th>
            </tr>
            </thead>
            ...
            <c:if test="${list != null}">
                <c:forEach items="${list}" var="p2p">
                    <tr>
                        <td title="${p2p.p2pFinancingInformation.finacingName }">${fns:abbr(p2p.p2pFinancingInformation.finacingName,20 )}</td>
                        ...
                        <td>
                            ...
                            <c:if test="${p2p.p2pFinancingInformation.currentStage == 900}">
                                <button class="btn yhk">已完成</button>
                            </c:if>
                        ...
                        <td>
                            <c:if test="${p2p.p2pFinancingInformation.givingOutState == 1 || p2p.p2pFinancingInformation.givingOutState == 2}">
                                <fmt:formatDate value="${p2p.p2pFinancingInformation.givingOutTime}"/>
                            </c:if>
                            <c:if test="${p2p.p2pFinancingInformation.givingOutState == 3 && p2p.p2pFinancingInformation.currentStage == 700}">
                                <fmt:formatDate value="${p2p.p2pFinancingInformation.givingOutTime}"/>&nbsp;&nbsp;&nbsp;&nbsp;
                                <button class="btn lr"
                            ...
                            <c:if test="${p2p.p2pFinancingInformation.givingOutState == 0 && p2p.p2pFinancingInformation.currentStage == 700}">
                                <button class="btn lr"
                                        onclick="viewModal('${p2p.financingInformationId}','${p2p.p2pFinancingInformation.finacingName}','${p2p.investmentSumAmount}')">
                                    录入
                                </button>
                    ...
                    <div class="modal-content">
                        ...
                        <form id="defaultForm" action="${ctx}/user/Institution/entryTime" method="post"
                            ...
                            <div class="modal-body">
                                ...
                                <div style="margin-top: 12px"><span style="float: left;">转账凭证：</span><input type="file"
                                                                                                            id="file"
                                                                                                            name="file"
                                                                                                            required="required"
                                                                                                            style="float: left;"
                            ...
                            <div class="modal-footer">
                                <input type="button" class="btn btn-primary" value="确   定" onclick="submitListen()"/>
