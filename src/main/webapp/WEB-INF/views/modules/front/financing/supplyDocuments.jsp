...
<div class="zh-con rzsq_con">
    <div class="renzheng">
        <p class="rzsq_pjcl">融资申请：评级材料</p>
        <h4 class="zhai-mima rz-h4">补充材料提交</h4>
        <ul class="cltj_scList" id="qycwcl-list">


        </ul>
        <div class="btn-box">
            <form action="${ctx}/document/submitSupply" method="post" id="form1">
                <input type="hidden" name="token" value="${token}"/>
                <input type="hidden" id="fid" name="financingInformationId" value="${financingInformationId}"/>
            </form>
...
<script>
    var fid = '${financingInformationId}';
    var arr = [];
    //加载模板
    $.ajax({
        async: false,
        type: 'POST',
        url: "${ctx}/document/loadSupplyTemp",
        data: {
    ...
    $.each(inps, function (index, item) {
        $(this).fileinput({
            language: 'zh',//设置语言
            uploadUrl: '${ctx}/document/uploadSupply?rateDocumentTempId=' + $(this).attr("data-id") + '&financingInformationId=' + fid,//ajax请求地址
            ...
            if (item.tempAddress) {
                if (item.tempExplain) {
                    newHtml += '<li class="clearfix">' +
                        ...
                        '<input id="as' + index + '" type="file" name="file" class="file-loading form-control" data-id="' + item.id + '" multiple accept="image/bmp,image/jpeg,image/jpg,image/png">' +
                        '</div>' +
