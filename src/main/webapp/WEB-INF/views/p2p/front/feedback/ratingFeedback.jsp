...
<div class="position-paper">
    <div class="position-paper-content">
        <div class="position-paper-btn">
            <a
                    href="${ctx}/financing/p2pFinancingRatingInfo/downLoadCreditReport?fid=${rating.p2pFinancingInformation.id}">
                <button>下载报告</button>
            </a>
        </div>
        <div class="position-paper-text">
            <h3>信用等级征求意见书</h3>
            <p>
                受贵公司委托,作为贵公司${rating.p2pFinancingInformation.finacingName}的评级方，在遵循了国家有关法律法规和公司有关制度的基础上，履行了既定的评级制度，规程，采用了应该使用的评级，评审手段和方法，得出了严谨的、务实的评级结论，评定贵公司债项级别为：${rating.ratingLevel}
            </p>
            <p>
        ...
        <div class="position-paper-text2">
            ...
            <p>大公信用数据有限公司：</p>
            <p>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;我公司已收到贵公司对${rating.p2pFinancingInformation.finacingName}的评级结果，并对评级结果表示：</p>
            <form id="defaultForm" action="${ctx}/feedback/RateReceipt/receiptRecord" method="post"
                  enctype="mulitipart/form-data" class="clearfix">
                <input type="hidden" name="token" value="${token}"/>
                <input type="hidden" id="fid" name="financingInformationId"
                       value="${rating.p2pFinancingInformation.id}"/>
                <div>
...
<script type="text/javascript">
    ...
    //初始化插件
    var fid = '${rating.p2pFinancingInformation.id}';
    initialize();
    function initialize() {
        $('#as').diyUpload({
            url: '${ctx}/feedback/RateReceipt/uploadReviewDoc?financingInformationId=' + fid,
