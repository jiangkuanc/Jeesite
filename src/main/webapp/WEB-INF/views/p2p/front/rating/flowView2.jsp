...
<head>
    ...
    <link rel="stylesheet" href="${ctxStatic}/dist/css/diyUpload.css"
          type="text/css">
    ...
    <script type="text/javascript"
            src="${ctxStatic}/diyUpload/js/webuploader.html5only.min.js"></script>
    <style>
        .zhzl li p a {
            color: #909090
        }
    ...
    <script type="text/javascript">
        ...
        function uploadDoc(id, tempName, financingInfoId, rateDocTempId) {
                ...
                if (p2p.p2pRateDocumentTemplate.id == rateDocTempId) {
                    ...
                    for (var i in p2p.p2pDocumentUploadRecordList) {
                        ...
                        str += '<li id=' + doc.id + ' class="">'
                            + '<div class="viewThumb"><img src="' + doc.fileAddress
                            + '" alt=""/></div>'
            ...
            $('#as').diyUpload(
                {
                    url: '${ctx}/document/documentUpload?rateDocumentTempId='
                    + tempId + '&financingInformationId=' + fid,
        ...
        function change() {
            if ($('#dtb').html().toString().trim() == '') {
                $.ajax({
                    type: 'POST',
                    url: '${ctx}/rating/front/updateDocState',
                    data: {
                        id: fid2
                    },
                    success: function () {
...
<div class="zh-right pull-left jianjie shenq"
    ...
    <p class="shenqp pull-left">
        您好,<span>${p2pRegUserCertify.realName}</span>,您的评级材料出现问题，请重新进行提交
    </p>
    <br/>
    <br/> <input type="hidden" id="path"
                 value="${fns:getConfig('filePath')}"/>
    <h2>申请进度：</h2>
    <div class="onlyone">
        <img src="${ctxStatic}/dist/images/jindu2/4.jpg" " alt=""
        style="margin-left: 7%;margin-top: 5%"> <span class="span1"><br>
    ...
    <div class="tabs">
        <ul>
            ...
            <li class="two border" id="commit"><span>待提交的材料</span>
            </li>
        ...
        <div class="shu">
            ...
            <div class="one">
                <table class="table table-hover">
                    ...
                    <tbody id="dtb">
                    <c:forEach items="${rateDocument2}" var="doc2">
                        <c:choose>
                            <c:when test="${doc2.p2pRateDocumentTemplate.tempType == 3}">
                                <tr id="tr${doc2.id}">
                                    <td>${doc2.p2pRateDocumentTemplate.tempName}</td>
                                    <td><fmt:formatDate value="${doc2.uploadTime}"
                                                        pattern="yyyy-MM-dd"/>
                                    </td>
                                    <td>待修改</td>
                                    <td><a
                                            href="javascript:sendForm('${doc2.financingInformationId}','${doc2.id}')">修改</a>
                                    </td>
                            ...
                            <c:otherwise>
                                <tr id="tr${doc2.id}">
                                    ...
                                    <td>待提交</td>
                                    <td><a
                                            href="javascript:uploadDoc('${doc2.id}','${doc2.p2pRateDocumentTemplate.tempName}','${doc2.financingInformationId}','${doc2.p2pRateDocumentTemplate.id}')">提交</a>
                                    </td>
                                    <td>${doc2.rejectReason}</td>
                                </tr>
                            </c:otherwise>
    ...
    <div class="modal-dialog modal-lg" style="width: 1000px;top:-300px;">
        <div class="modal-content">
            ...
            <div class="modal-body" style="width: 1000px;height: 500px">
                <label class="col-lg-3 control-label" id="modalName"></label> <input
                    id="file" type="file" name="file"
                    class="webuploader-element-invisible" multiple="multiple"
                    accept="image/*">

                <div id="as"></div>
                <div class="parentFileBox" style="width: 1349px;">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-default"
                        style="width:6%;height:35px;" data-dismiss="modal" onclick="edg()">关闭
                </button>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    function sendForm(fid, rdid) {
        post('${ctx}/rating/front/preUpdateApp', {
