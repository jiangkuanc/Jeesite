...
<body>
<form:form id="searchForm" modelAttribute="user" action="${ctx}/user/userList/assetList" method="post"
    ...
    <ul class="ul-form">
        ...
        <li class="btns"><a class="btn btn-primary" type="button"
                            href="${ctx}/user/userList/assetRegisterForm">添加评估公司</a></li>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed listtable">
    ...
    <c:forEach items="${page.list}" varStatus="status" var="user">
        <tr>
            ...
            <td>
                <a href="${ctx}/user/userList/assetBaseInfo?userid=${user.id}">
                        ${user.p2pAssetValuation.name}
                </a>
            </td>
            <td title="${user.p2pAssetValuation.address }">${fns:abbr(user.p2pAssetValuation.address,50)}</td>
            <td>${fns:getDictLabel(user.p2pUserInformation.userFrom, 'user_from', '')}</td>
            <td>${fns:getDictLabel(user.p2pAssetValuation.isCertify, 'yes_no', '')}</td>
            <td>${user.p2pAssetValuation.businessScope }</td>
            <td><a onclick="modelView('${user.p2pAssetValuation.cooperationPlan }')">战略合作协议</a></td>
            <td>${user.name}</td>
            <td>${user.phone}</td>
            <td>${user.email}</td>
