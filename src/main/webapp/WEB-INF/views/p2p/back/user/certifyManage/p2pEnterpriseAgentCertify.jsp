...
<body>
<div class="title"><h3>企业代理人身份认证审核页</h3></div>
...
<table class="table table-striped table-bordered table-condensed mytable">
    ...
    <tr>
        ...
        <td>姓名与身份证是否匹配</td>
        <td>
            <font color='red'>${fns:getDictLabel(p2pRegUserCertify.autoCertifyState, 'yes_no', '')}</font>
        </td>
    </tr>
    </tbody>
</table>
<form id="submitForm" action="${ctx}/user/certifyManage/enterpriseAgentAudit" method="post">
    <h4>身份信息认证</h4>
    ...
    <table id="contentTable" class="table table-striped table-bordered table-condensed regUserCertify">
        ...
        <tr>
            ...
            <td>委托书</td>
            <td>企业委托书扫描件</td>
            <td><img style="width:100px" src="${p2pRegUserCertify.powerOfAttorneyView }"></td>
            <td><fmt:formatDate value="${p2pRegUserCertify.submitTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
            <td>
            ...
            <tr>
                ...
                <td>
                    <select name="auditOpinion" class="input-medium-select">
                        ...
                        <!-- 身份认证失败的时候，不显示成功选项，且不能点击确认 -->
                        <c:if test="${p2pRegUserCertify.autoCertifyState == 1}">
    ...
    <table class="table table-striped table-bordered table-condensed" style="text-align: center;table-layout: fixed;">
        ...
        <c:forEach items="${page.list}" varStatus="status" var="p2pAuditRecord">
            <tr>
                ...
                <td rowspan="4"><fmt:formatDate value="${p2pAuditRecord.auditTime }"
                                                pattern="yyyy-MM-dd HH:mm:ss"/></td>
                <td rowspan="4">${p2pAuditRecord.user.name}</td>
                <td rowspan="4">${fns:getDictLabel(p2pAuditRecord.auditOpinion, 'audit_state', '')}</td>
                <td style="word-break:break-all; word-wrap:break-word;" rowspan="4">${p2pAuditRecord.auditResult }</td>
                <td>身份证正面</td>
            ...
            <tr>
                <td>企业委托书扫描件</td>
                <td><img style="width:100px" src="${p2pAuditRecord.powerOfAttorney }"></td>
                <td><fmt:formatDate value="${p2pAuditRecord.submitTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                <td>${fns:getDictLabel(p2pAuditRecord.attorneyIsReject, 'yes_no', '')}</td>
                <td>${p2pAuditRecord.attorneyRejectReason}</td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
