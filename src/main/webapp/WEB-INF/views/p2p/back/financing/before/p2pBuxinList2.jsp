...
<ul class="nav nav-tabs">
    <li><a href="${ctx}/before/buxin/buxinList1">抵质押增信借贷产品列表</a></li>
    <li class="active"><a href="${ctx}/before/buxin/buxinList2">担保增信借贷产品列表</a></li>
</ul>
<form:form id="searchForm" modelAttribute="p2pFinancingInformation" action="${ctx}/before/buxin/buxinList2"
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <c:forEach items="${page.list}" varStatus="status" var="p">
        <tr>
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                        <shiro:hasPermission name="credit:check">
                            <a href="${ctx}/before/buxin/assetValuation?fiId=${p.id}">查看</a>
                        </shiro:hasPermission>
                    </c:otherwise>
                </c:choose>
            </td>
            <!-- 担保公司名称 -->
            <td>
                <a href="${ctx}/user/userList/guaranteeBaseInfo?userid=${p.p2pGuarantee.userId}">
                        ${p.p2pGuarantee.name }
                </a>
            </td>
            <!-- 担保审核状态 -->
            <td>${fns:getDictLabel(p.p2pGuaranteeRecord.isReject, 'buxin_state', '')}</td>
            <!-- 担保操作 -->
            <td>
                <c:choose>
                    <c:when test="${p.p2pGuaranteeRecord.isReject == 0 }">
                        <shiro:hasPermission name="credit:guarantee:examine">
                            <a href="${ctx}/before/buxin/guarantee?fiId=${p.id}">审核</a>
                        </shiro:hasPermission>
                    </c:when>
                    <c:otherwise>
                        <shiro:hasPermission name="credit:guarantee:see">
                            <a href="${ctx}/before/buxin/guarantee?fiId=${p.id}">查看</a>
                        </shiro:hasPermission>
                    </c:otherwise>
                </c:choose>
            </td>
