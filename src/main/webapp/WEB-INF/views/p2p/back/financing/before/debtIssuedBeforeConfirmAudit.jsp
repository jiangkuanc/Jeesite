...
<form action="${ctx}/financing/p2pBeforeMatchDebtState/debtFinal" id="debtFinal" method="post">
    <h4>市场意见</h4>
    <input type="hidden" name="finacingInformationId" value="${vo.id}"/>
    <table class="table table-striped table-bordered table-condensed mytable opinion" id="table8">
        <tr>
            <td>操作人</td>
            <td>
                <input type="text" value="${fns:getUser().name}" name="operateId" readonly="readonly"/>
            </td>
            <td>操作时间</td>
            <td>
        ...
        <tr>
            <td>市场发布意见</td>
            <td>
                <select id="progress_stage" name="progress_stage" class="input-medium-select"
                        onchange="changeInput(this.options[this.options.selectedIndex].value)">
            ...
            <td>不同意发布意见说明</td>
            <td>
                <textarea type="text" name="handleResult" id="handleResult" value=""></textarea>
            </td>
        </tr>
    </table>
