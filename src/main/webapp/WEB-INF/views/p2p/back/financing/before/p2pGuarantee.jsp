...
<body>
<div class="title"><h3>资产担保审核页</h3></div>
...
<table  class="table table-striped table-bordered table-condensed mytable">
    <tbody>
        <tr>
            <td>评估结果时间</td><td><fmt:formatDate value="${p2pAssetValuationRecord.commitTime }" pattern="yyyy-MM-dd"/></td>
            <td>操作人</td><td>${p2pAssetValuationRecord.p2pAssetValuation.user.name }</td>
        </tr>
        <tr>
            <td>资产评估额（元）</td><td><fmt:formatNumber pattern="#,##0.00">${p2pAssetValuationRecord.assetValuation}</fmt:formatNumber></td>
            <td>资产评估报告</td><td>
...
<table class="table table-striped table-bordered table-condensed mytable">
    ...
    <tr>
        <td>担保结果时间</td>
        <td><fmt:formatDate value="${p2pGuaranteeRecord.commitTime }" pattern="yyyy-MM-dd"/></td>
        <td>操作人</td>
        <td>${p2pGuaranteeRecord.p2pGuarantee.user.name }</td>
    </tr>
    <tr>
        <td>担保函</td>
        <td>
            <a onclick="modelView('${p2pGuaranteeRecord.letterIntent }')">
                ${p2pGuaranteeRecord.letterIntentName }
            </a>
        </td>
        <td>担保合同</td>
        <td>
            <a onclick="modelView('${p2pGuaranteeRecord.guaranteeContract }')">
                ${p2pGuaranteeRecord.guaranteeContractName }
            </a>
        </td>
    </tr>
    </tbody>
</table>
<form id="submitForm" action="${ctx}/before/buxin/guaranteeAudit" method="post">
    <h4>审核意见</h4>
    <input name="id" type="hidden" value="${p2pGuaranteeRecord.id }">
    <input name="financingInformationId" type="hidden" value="${p2pGuaranteeRecord.financingInformationId }">
    <input name="id2" type="hidden" value="${p2pAssetValuationRecord.id }">
    ...
    <c:choose>
        <c:when test="${p2pGuaranteeRecord.isReject == 0}">
        ...
        <c:otherwise>
            <table id="contentTable" class="table table-striped table-bordered table-condensed opinion">
                <tr>
                    <td>审核意见</td>
                    <td>${fns:getDictLabel(p2pGuaranteeRecord.isReject , 'buxin_state', '')}</td>
                    <td>不通过原因</td>
                    <td>${p2pGuaranteeRecord.rejectReason }</td>
                </tr>
                <tr>
                    <td>操作人</td>
                    <td>${fns:getUserById(p2pGuaranteeRecord.rejectPerson).name }</td>
                    <td>操作时间</td>
                    <td><fmt:formatDate value="${p2pGuaranteeRecord.rejectTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                </tr>
            </table>
        </c:otherwise>
    </c:choose>
