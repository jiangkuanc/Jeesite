...
<form:form id="searchForm" modelAttribute="p2pFinancingInformation"
           action="${ctx}/inFinancing/findStandardAuditListPage" method="post" onsubmit="return checksubmit()"
    ...
    <ul class="ul-form">
        ...
        <li><label>借贷产品状态：</label>
            <form:select path="currentStage" class="input-medium-select">
                ...
                <form:option value="999">已流标</form:option>
            </form:select>
        </li>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed listtable">
    ...
    <c:forEach items="${page.list}" varStatus="status" var="p2pFinancingInformation">
        <tr>
            ...
            <c:if test="${p2pFinancingInformation.currentStage == 901}">
                <td>
                <shiro:hasPermission name="flow:audit">
                    <a href="${ctx}/inFinancing/getP2pStandardAuditPage?id=${p2pFinancingInformation.id}">流标审核</a></td>
                </shiro:hasPermission>
            </c:if>
            <c:if test="${p2pFinancingInformation.currentStage == 999}">
                <td><a href="${ctx}/inFinancing/getP2pStandardAuditPage?id=${p2pFinancingInformation.id}">查看</a></td>
            </c:if>
        </tr>
    </c:forEach>
    </tbody>
