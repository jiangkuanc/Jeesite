        ...
        <tr>
            ...
            <td>
                <a onclick="modelView('${p2pAssetValuationRecord.evaluationContract }')" href="#">
                        <%-- 						<a  onclick="modelView('${ctx}/image/test?path=${fns:getConfig('filePath')}${p2pAssetValuationRecord.evaluationContract }')" href="#"> --%>
                    查看
                </a>
            </td>
            <td></td>
...
<form action="${ctx}/financing/p2pBeforeMatchDebtState/debtRealFinal" id="debtFinal" method="post"
      modelAttribute="p2pBeforeMatchDebtState">
    <h4>发布意见</h4>
    <table id="table09" class="table table-striped table-bordered table-condensed opinion">
        <tr style="text-align: center;">
            <input type="hidden" name="financingInformationId" value="${vo.id}"/>
            <c:if test="${vo.financingAmount >= vo.applicationAmount}">
                <input type="hidden" name="financingAmount" value="${vo.applicationAmount}"/>
            </c:if>
            <c:if test="${vo.financingAmount < vo.applicationAmount}">
                <input type="hidden" name="financingAmount" value="${vo.financingAmount}"/>
            </c:if>
            <td>发布人</td>
            <td>
                <input type="text" value="${fns:getUser().name}" name="operatorName" readonly="readonly"/>
                <input type="hidden" value="${fns:getUser().id}" name="operatorId"/>
            </td>
            <td>发布时间</td>
            <td>
                <input id="publishTime" style="cursor: pointer;" name="publishTime" type="text" readonly="readonly"
        ...
        <tr style="text-align:center;">
            ...
            <td>
                <!-- 					<input type="text" value="" name="handleResult"/> -->
