...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $("#riskManagementEndForm").validate({
...
<h4>借贷产品终止</h4>
<form id="riskManagementEndForm" action="${ctx}/inFinancing/RiskManagementEnd" method="post">
    <table class="table table-striped table-bordered table-condensed mytable">
        <tbody>
        <tr>
            <td>终止人员<input type="hidden" name="financingInformationId" value="${p2pFinancingInformation.id}"></td>
            <td><input type="hidden" name="userId" value="${fns:getUser().id}"/>${fns:getUser().name}</td>
            <td>终止时间<input type="hidden" name="applicantsId" value="${p2pFinancingInformation.applicantsId}"></td>
            <td><fmt:formatDate value="${date}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
        </tr>
        <tr>
            <td>终止原因<input type="hidden" name="markType" value="${p2pFinancingInformation.markType}"></td>
