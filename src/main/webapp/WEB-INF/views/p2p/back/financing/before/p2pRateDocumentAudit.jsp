            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.length > 0) {
                        var msg = "";
                        for (i in data) {
                            msg += data[i].p2pRateDocumentTemplate.tempName + "、"
                        }
...
<div id="myTabContent" class="tab-content">
    <div class="tab-pane fade in active" id="home">
        ...
        <table id="contentTable" class="table table-striped table-bordered table-condensed">
            ...
            <tbody>
            <c:forEach items="${list2}" varStatus="status" var="p2pRateDocumentAudit" begin="0" end="6">
                <tr style="text-align:center;" class="tr1">
                    ...
                    <td>
                        <a href="${ctx}/financing/downloads?financingInformationId=${list[status.index].financingInformationId}&rateDocumentTempId=${list[status.index].rateDocumentTempId}">下载</a>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
    ...
    <div class="tab-pane fade" id="ios">
        <table id="contentTable" class="table table-striped table-bordered table-condensed">
            ...
            <tbody>
            <c:forEach items="${list2}" varStatus="status" var="p2pRateDocumentAudit" begin="7">
                <tr style="text-align:center;" class="tr2">
                    ...
                    <td id="cailiaoML${status.index }">
                        <c:choose>
                            <c:when test="${status.index == fn:length(list2)-1}">
                                <a onclick="model_enterpriseBaseInfo('${financingInformationId}')">
                                        ${list2[status.index].tempName}
                                </a>
                            </c:when>
                            <c:otherwise>${list2[status.index].tempName}</c:otherwise>
                        </c:choose>
                    </td>
                    <!-- 材料提交说明 -->
                    <td>
                    ...
                    <td>
                        <fmt:formatDate value="${list2.uploadTime}" pattern="yyyy-MM-dd"/>
                    </td>
                    <form action="${ctx}/document/p2pRateDocumentAuditRecord/addOne" id="formAdd${status.index }"
                          method="post">
                        <td id="isReject${status.index }">
