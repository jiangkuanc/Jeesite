        ...
        $(document).ready(function () {
            ...
            function _fresh() {
                for (var j in data) {
                    ...
                    var currentStage = data[j]['currentStage'];//债项状态
                    var nowtime;//当前时间
                    var endtime;//结束时间
            ...
            //********************进算进度******************
            for (var k in data) {
                var ids = "jindu" + data[k]['id'];
                var hasRaiseAmount = data[k]['hasRaiseAmount'];
                if (hasRaiseAmount == null || hasRaiseAmount == undefined) {
                    hasRaiseAmount = 0;
                }
                var financingAmount = null;
                if (data[k]['financingAmount'] >= data[k]['applicationAmount']) {
                    financingAmount = data[k]['applicationAmount'];
                } else {
                    financingAmount = data[k]['financingAmount'];
                }
...
<body>
<form:form id="searchForm" modelAttribute="p2pFinancingInformation" action="${ctx}/inFinancing/getInFinancingPage"
...
<table id="contentTable" class="table table-striped table-bordered table-condensed listtable">
    ...
    <c:forEach items="${page.list}" varStatus="status" var="p2pFinancingInformation">
        <tr>
            ...
            <td id="jindu${p2pFinancingInformation.id}">${(p2pFinancingInformation.hasRaiseAmount/p2pFinancingInformation.financingAmount)*100}%</td>
