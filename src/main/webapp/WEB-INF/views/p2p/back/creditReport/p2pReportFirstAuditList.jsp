...
<form:form id="searchForm" modelAttribute="p2pReportInformationAudit"
           action="${ctx}/creditReport/reportFirstAudit/p2pReportFirstAuditList" method="post"
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <c:forEach items="${page.list}" varStatus="status" var="ria">
        <tr>
            ...
            <!--初审意见-->
            <td>${fns:getDictLabel(ria.firstOpinion, 'pass_nopass', '')}</td>
            <!--初审员-->
            <td>${fns:getUserById(ria.firstStaff).name}</td>
            <!--初审时间-->
            <td><fmt:formatDate value="${ria.firstTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
            <!--操作-->
            <td>
                <c:choose>
                    <c:when test="${ria.reportAuditState == 1 }">
                        <a href="${ctx }/creditReport/reportFirstAudit/p2pReportFirstAuditForm?id=${ria.id}&&type=1">初审</a>
                    </c:when>
                    <c:otherwise>
                        <a href="${ctx }/creditReport/reportFirstAudit/p2pReportFirstAuditForm?id=${ria.id}&&type=2">查看</a>
                    </c:otherwise>
                </c:choose>
