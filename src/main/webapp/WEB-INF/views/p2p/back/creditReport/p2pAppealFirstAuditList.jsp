...
<form:form id="searchForm" modelAttribute="p2pInformationAppeal"
           action="${ctx}/creditReport/appealFirstAudit/p2pAppealFirstAuditList" method="post"
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <c:forEach items="${page.list}" varStatus="status" var="ia">
        <tr>
            ...
            <!--初审意见-->
            <td>${fns:getDictLabel(ia.firstReviewOpinion, 'pass_nopass', '')}</td>
            <!--初审员-->
            <td>${fns:getUserById(ia.firstReviewStaff).name}</td>
            <!--初审时间-->
            <td><fmt:formatDate value="${ia.firstReviewTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
            <!--操作-->
            <td>
                <c:choose>
                    <c:when test="${ia.appealAuditState == 1 }">
                        <a href="${ctx }/creditReport/appealFirstAudit/p2pAppealFirstAuditForm?id=${ia.id}&&type=1">初审</a>
                    </c:when>
                    <c:otherwise>
                        <a href="${ctx }/creditReport/appealFirstAudit/p2pAppealFirstAuditForm?id=${ia.id}&&type=2">查看</a>
                    </c:otherwise>
                </c:choose>
            </td>
        </tr>
