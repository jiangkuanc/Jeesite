            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.length > 0) {
                        var msg = "";
                        for (i in data) {
                            msg += (data[i].p2pRateDocumentTemplate.tempName + "、").replace("<br/>", "").replace("<br>", "")
                        }
...
<div id="myTabContent" class="tab-content">
                            ...
                            <c:choose>
                                <c:when test="${p2pRateDocumentAudit.p2pRateDocumentTemplate.tempType == 3}">
                                    <a onclick="model_enterpriseBaseInfo('${financingInformationId}')">
                                            <%-- ${list[status.index].p2pRateDocumentTemplate.tempName} --%>评级项目基本信息
                                    </a>
                                </c:when>
    ...
    <form action="${ctx}/rating/ratingDocumnetAudit/audit" id="renzhengFinal" method="post">
