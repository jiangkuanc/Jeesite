...
<body>
<div class="title"><h3>申请评级项目详情</h3></div>
...
<table class="table table-striped table-bordered table-condensed mytable">
    ...
    <tr>
        <td>企业名称</td>
        <td>${p2pRatingServiceInfo.p2pEnterpriseCertify.enterpriseName }</td>
        <td>注册地址</td>
        <td>${p2pRatingServiceInfo.p2pEnterpriseCertify.registeredAddress }</td>
    </tr>
    <tr>
        <td>企业规模</td>
        <td>${fns:getDictLabel(p2pRatingServiceInfo.p2pEnterpriseCertify.enterpriseScale , 'enterprise_scale', '')}</td>
        <td>企业性质</td>
        <td>${fns:getDictLabel(p2pRatingServiceInfo.p2pEnterpriseCertify.enterpriseNature , 'enterprise_nature', '')}</td>
    </tr>
    <tr>
        <td>所属行业</td>
        <td>${p2pRatingServiceInfo.p2pEnterpriseCertify.p2pIndustryclassi.classinameCn}</td>
        <td>实缴资本</td>
        <td><fmt:formatNumber
                pattern="#,##0.00">${p2pRatingServiceInfo.p2pEnterpriseCertify.realCapital}</fmt:formatNumber>元
        </td>
    </tr>
    <tr>
        <td>所在地</td>
        <td>${area }</td>
        <td>经营区域</td>
        <td>${p2pRatingServiceInfo.p2pEnterpriseCertify.businessArea}</td>
    </tr>
    <tr>
        <td>营业执照登记机关</td>
        <td>${p2pRatingServiceInfo.p2pEnterpriseCertify.registrationAuthority}</td>
        <td>营业执照登记状态</td>
        <td>${fns:getDictLabel(p2pRatingServiceInfo.p2pEnterpriseCertify.registrationState, 'registration_state', '')}</td>
    </tr>
    <tr>
        <td>营业执照准核日期</td>
        <td><fmt:formatDate value="${p2pRatingServiceInfo.p2pEnterpriseCertify.quasiNuclearDate}"
                            pattern="yyyy-MM-dd"/></td>
        <td>是否存在境外交易</td>
        <td>${fns:getDictLabel(p2pRatingServiceInfo.p2pEnterpriseCertify.isOffshoreTransaction, 'yes_no', '')}</td>
    </tr>
    <tr>
        <td>企业经营范围</td>
        <td colspan="3">${p2pRatingServiceInfo.p2pEnterpriseCertify.businessScope}</td>
    </tr>
    </tbody>
</table>
...
<table class="table table-striped table-bordered table-condensed">
    ...
    <c:forEach items="${list}" varStatus="status" var="p2pRateDocumentAudit">
        <tr style="text-align:center;">
            ...
            <td>
                <c:choose>
                    ...
                    <c:when test="${p2pRateDocumentAudit.p2pRateDocumentTemplate.tempType == 3}">
                        <a onclick="model_enterpriseBaseInfo('${p2pRatingServiceInfo.id}')">
            ...
            <td>
                <c:choose>
                    <c:when test="${listAll.size() > 1}">
                        <c:if test="${status.index >=  listAll.size()-1}">--</c:if>
                        <c:if test="${status.index <  listAll.size()-1}">
                            <shiro:hasPermission name="scan:download">
