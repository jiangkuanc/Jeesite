...
public class LinkService extends CrudService<LinkDao, Link> {
    ...
    public List<Link> findPaListShiPin(Link link) {
        return linkDao.findPaListShiPin(link);
    ...
    @Transactional(readOnly = false)
    public List<Link> selectCurrentVideoDetail(Link link) {
        return linkDao.selectCurrentVideoDetail(link);
    }
